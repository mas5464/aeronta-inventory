...
- **Local full-stack Docker deploy** (`docker-compose.yml` at repo root, project name `trax-io-planner`): `docker compose up --build` runs the **BFF** (`deploy/bff.Dockerfile`, uv-built over the four path-dep packages, booting in **~14s** from a complete precomputed snapshot dir — `deploy/_local_extract/emro_net_full_snapshot`, gitignored real **full-network eMRO data (58.9K planning-active keys)** mounted via `PLANNER_SNAPSHOT_DIR`; regenerate with `trax-io-precompute`) + the **UI** (`apps/web/Dockerfile` → nginx, the single frontend) which reverse-proxies `/v1` → BFF (same-origin, no CORS). Open **http://localhost:8089** (UI); BFF debug on `:8001`. `docker compose down` to stop. Scoped to this project only — never touches `oracle19c`/MySQL.
