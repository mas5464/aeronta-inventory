...
| `services/forecasting` | `cd services/forecasting && uv run --extra dev pytest` — slice A `StatisticalProjector` (statsforecast, intermittent; first `uv sync` compiles numba) + slice B `GradientBoostedProjector` (sklearn HistGradientBoosting, moderate/high) | `uv run --extra dev ruff check .` |
...
| `apps/planner-ui` (React/TS — first frontend) | `cd apps/planner-ui && npm install && npm test` (Vitest, 25 tests); `npm run build` (tsc + vite) | `tsc -b` (no ruff — JS/TS) |
...
- The **Planner-UI React frontend** (`apps/planner-ui/` — the repo's first frontend; React 18 + TS + Vite 5 + Vitest 2 + CSS Modules, node-20.17-safe) renders the BFF via a typed `PlannerClient` (`HttpPlannerClient` + `FakePlannerClient`): queue → provenance detail → approve/reject/defer → kill switch. **iCloud caveat: `node_modules` is gitignored and the app was built/tested in a scratchpad outside iCloud, source committed; run `npm install` before `npm test`.** `VITE_FAKE=1 npm run dev` runs offline; `VITE_BFF_URL=… npm run dev` against a live BFF. Scope = core loop; bulk-approve UI / history view / digest / settings / auth deferred ([ADR-0012](docs/adr/2026-06-28-0012-planner-ui-react-frontend.md)).
