...
| `apps/planner-ui` (React/TS — first frontend) | `cd apps/planner-ui && npm install && npm test` (Vitest, 110 tests); `npm run build` (tsc + vite); **UAT plan: [apps/planner-ui/UAT.md](apps/planner-ui/UAT.md)** (run before every release — manual cases + automated-test traceability) | `tsc -b` (no ruff — JS/TS) |
