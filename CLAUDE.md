...
| `apps/planner-ui` (React/TS — first frontend) | `cd apps/planner-ui && npm install && npm test` (Vitest, 46 tests); `npm run build` (tsc + vite) | `tsc -b` (no ruff — JS/TS) |
...
- The **Planner-UI React frontend** (`apps/planner-ui/` — the repo's first frontend; React 18 + TS + Vite 5 + Vitest 2 + CSS Modules, node-20.17-safe) renders the BFF via a typed `PlannerClient` (`HttpPlannerClient` + `FakePlannerClient`): queue → provenance detail → approve/reject/defer → **bulk-approve filter bar** → **inline writeback-history timeline + rollback** → kill switch, with double-submit + stale-detail guards in `usePlanner`. `node_modules` is gitignored — run `npm install` before `npm test` (the repo now lives outside iCloud at `~/Projects/…`, so the old scratchpad workaround is retired). `VITE_FAKE=1 npm run dev` runs offline (seeded via `SAMPLE_SEED`/`SAMPLE_HISTORY`); `VITE_BFF_URL=… npm run dev` against a live BFF. Scope = core loop + guards/bulk-approve/history; routing/tabs, full WCAG, digest, settings, auth still deferred ([ADR-0012](docs/adr/2026-06-28-0012-planner-ui-react-frontend.md)).
