...
| `services/agent-spine` | `cd services/agent-spine && uv run --extra dev pytest` (add `--extra emro` for the `fake_emro` writeback + integration tests; `--extra cedar` for the `CedarAutonomyPolicy` tests; `--extra bff` **+ `--extra bvr`** for the `trax_io_spine.bff` BFF tests — the BFF app renders the #8 BVR report, jinja2 required; `--extra pdf` for the WeasyPrint test, which on macOS needs `DYLD_FALLBACK_LIBRARY_PATH=/opt/homebrew/lib` **and `python -m pytest`** — see `tests/bvr/test_pdf.py`; `--extra pg` for the `trax_io_spine.pg` Postgres suite — Docker required, testcontainers boots a real Postgres, skips clean without Docker) | `uv run --extra dev ruff check .` |
...
| `apps/web` (React/TS — the Trax Inventory Optimizer UI, the single frontend) | `cd apps/web && npm install && npm test` (Vitest, 288 tests); `npm run build` (tsc + vite); `npm run e2e` (Playwright, best-effort — 1 spec: Workbench accept-removes-row against a route-mocked BFF, no backend needed); **UAT plan: [apps/web/UAT.md](apps/web/UAT.md)** (run before every release) | `npm run lint` (eslint; no ruff — JS/TS) |
