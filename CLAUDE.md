...
- **Local full-stack Docker deploy** (`docker-compose.yml` at repo root, project name `trax-io-planner`): `docker compose up --build` runs the **BFF** (`deploy/bff.Dockerfile`, uv-built over the four path-dep packages, seeded from the sample extract) + the **UI** (`apps/planner-ui/Dockerfile` → nginx) which reverse-proxies `/v1` → BFF (same-origin, no CORS). Open **http://localhost:8088** (UI); BFF debug on `:8001`. `docker compose down` to stop. Scoped to this project only — never touches `oracle19c`/MySQL. Data = real engine output from the sample extract (differs from the `apps/planner-ui/UAT.md` fake seed; for the exact documented values run `VITE_FAKE=1 npm run dev`).
