...
def _rec(
    ...
    *,
    ...
    return Recommendation(
        ...
        recommended_location=("YOW" if rtype == RecommendationType.TRANSFER else None),
        current_stock=0,
        projected_demand=10.0,
        shortage_quantity=shortage,
        recommended_quantity=qty,
        estimated_cost_impact=Decimal(cost),
        aog_risk_level=aog,
        reason="r",
...
def _net(shortage: float) -> NetPosition:
    return NetPosition(
        ...
        window_days=30,
        available=0.0,
        expected_receipts_in_window=0.0,
        projected_demand=shortage,
        net=-shortage,
        shortage=shortage,
    )
...
def test_arbitration_transfer_fully_covers_drops_purchase() -> None:
    recs = [
        _rec(RecommendationType.TRANSFER, shortage=5, qty=5),
...
def test_arbitration_partial_transfer_leaves_residual_purchase() -> None:
    recs = [
        _rec(RecommendationType.TRANSFER, shortage=5, qty=3),
        _rec(
    ...
    out = arbitrate(recs, net=_net(5), min_order_qty=4)
    purchase = [r for r in out if r.type == RecommendationType.PURCHASE][0]
    assert purchase.recommended_quantity == 4.0
    assert purchase.shortage_quantity == 2.0
    assert purchase.estimated_cost_impact == Decimal("400")
    moq = next(
        constraint
        for constraint in purchase.applied_constraints
        if constraint.name == "minimum_order_quantity_action"
    )
    assert moq.value == "4"
    assert moq.binding is True
    assert moq.scope == "action"


def test_arbitration_shortage_drops_reduce_sell() -> None:
    recs = [
        _rec(RecommendationType.SELL, qty=10),
        _rec(RecommendationType.PURCHASE, shortage=5, qty=5),
    ]
    out = arbitrate(recs, net=_net(5))
...
def test_aog_active_forces_advisor() -> None:
    ...
    scored = AogRiskScorer().score(
        _rec(RecommendationType.PURCHASE, shortage=2, qty=2), context=ctx, net=_net(2)
    )
...
def test_confidence_stub_lower_than_real() -> None:
    ...
    stub = confidence_score(
        events_24mo=12, regime=Regime.INTERMITTENT, used_stub_inputs={"aog", "repair_tat"}
    )
    assert stub < real
    assert 0.0 <= stub <= 1.0
...
def test_suggest_tier_thresholds() -> None:
    assert (
        suggest_tier(criticality=1, unit_cost=100, delta_pct=0.0, active_aog=False)
        == AutonomyTier.ADVISOR
    )
    assert (
        suggest_tier(criticality=5, unit_cost=100, delta_pct=0.1, active_aog=False)
        == AutonomyTier.AUTONOMOUS
    )
    assert (
        suggest_tier(criticality=3, unit_cost=100, delta_pct=0.1, active_aog=False)
