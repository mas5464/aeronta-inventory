...
def _cp(lam: float) -> DemandProjection:
    return DemandProjection(
        mean_per_day=lam, std_per_day=lam**0.5, dist_kind="COMPOUND_POISSON",
        dist_params={"lambda": lam, "clump_p": 1.0}, historical_component=lam,
        scheduled_component=0.0, by_aircraft={}, by_task={}, basis_window_days=730,
    )


def _normal(mean: float, var: float) -> DemandProjection:
    return DemandProjection(
        mean_per_day=mean, std_per_day=var**0.5, dist_kind="NORMAL",
        dist_params={"mean": mean, "var": var}, historical_component=mean,
        scheduled_component=0.0, by_aircraft={}, by_task={}, basis_window_days=730,
    )
...
def test_s_S_floors_hold() -> None:
    rop, eoq, ss, mx = compute_s_S(
        projection=_cp(0.05), lead_mean=20.0, lead_var=0.0, service_level=0.95,
        ordering_cost=150.0, holding_cost_rate=0.25, unit_cost=100.0, min_order_qty=1,
    )
...
def test_R_Q_floors_and_minoq() -> None:
    rop, eoq, ss, mx = compute_R_Q(
        projection=_normal(1.0, 1.5), lead_mean=21.0, lead_var=4.0, service_level=0.92,
        ordering_cost=150.0, holding_cost_rate=0.25, unit_cost=50.0, min_order_qty=12,
    )
...
def test_constraints_shelf_life_clamp() -> None:
    res = apply_constraints(
        ...
        avg_daily_demand=0.2,  # cap = floor(0.6*30/0.2) = 90 -> no clamp
        min_order_qty=1,
    )
