    ...
    for p in data["parts"]:
        ...
        fs.seed(
            ...
            Criticality(
                tenant_id=tenant_id,
                pn=pn,
                raw_essentiality_code=str(p.get("tier", 4)),
                canonical_tier=int(p.get("tier", 4)),
                extract_date=_EXTRACT_DATE,
            ),
        )
        fs.seed(
        ...
        fs.seed(
            ...
            LeadTimeDistribution(
                ...
                vendor=vendor,
                condition="NEW",
                promised_lead_days=lead_mean,
                realized_mean_days=lead_mean,
                realized_p50_days=lead_mean,
                realized_p90_days=lead_mean * 1.3,
                realized_p99_days=lead_mean * 1.6,
                promised_vs_actual_delta_mean=0.0,
                n_observations=int(p.get("lead_obs", 10)),
                extract_date=_EXTRACT_DATE,
            ),
        )
