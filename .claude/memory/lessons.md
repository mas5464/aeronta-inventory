...
## Vercel Git auto-deploy from a monorepo root silently 404s prod on every push (2026-07-22)
**What happened:** All session deploys of `apps/web` were `vercel deploy --prod` run from inside `apps/web` (correct). But the Vercel project `aeronta-inventory` also had a **GitHub integration on `main`**. A routine `git push origin main` (a docs commit) triggered an **automatic** production deploy that built from the **repo root** — where there's no app — producing an empty deployment (`Builds: . [0ms]`) that grabbed the `aeronta-inventory.vercel.app` production alias. The whole site went 404: root, `/index.html`, AND `/v1/*` (the `/v1` 404 was the tell — it should 401 via `vercel.json`'s rewrite, so a 404 there means the serving deployment had no `vercel.json`, i.e. it wasn't built from `apps/web`).
**Fix:** `vercel promote <last-good-deployment-url>` restored the alias in seconds (root 200, `/v1` 401). Then, per the user, `vercel git disconnect` (CLI) to stop auto-deploys entirely — we deploy via CLI from `apps/web` anyway. The proper alternative (keeps CD) is setting the project's **Root Directory = `apps/web`** in the dashboard, but that's dashboard/REST-only — `vercel project` has no update subcommand and the CLI's stored session isn't usable as a REST bearer.
**Rules:**
- **A Vercel project whose app is in a subdirectory of a monorepo MUST have its Root Directory set to that subdir** — otherwise Git auto-deploys build the wrong thing. If you deploy via CLI from the subdir, the Git integration is a footgun: disconnect it or set the root dir.
- **When a previously-working live URL suddenly 404s after a push, suspect a hijacked production alias first** (`vercel ls <project>` shows a fresh deployment with a `[0ms]` build; `vercel inspect <alias>` shows what it points to). Recover with `vercel promote`, don't rebuild from scratch.
- **`/v1` (or any rewrite-backed path) returning 404 instead of its normal auth code means the served build lacks `vercel.json`** — a fast signal that the wrong artifact is live.

**C4 addendum (2026-07-23):** once an app directory imports something OUTSIDE itself in the monorepo — `apps/web` and `apps/site` both now pull `../../packages/tailwind-preset/*` — plain `vercel deploy --prod` run from that app directory breaks a different way: the Vercel CLI uploads only the CWD, so the *remote* build can't see `packages/tailwind-preset` and fails outright (not a silent wrong-artifact 404, a hard build error). Deploy prebuilt instead: `vercel build --prod && vercel deploy --prebuilt --prod` from the app directory — the build runs locally where the monorepo-relative import resolves, and only the built output is uploaded. See [deploy/C4_ROLLOUT.md](../../deploy/C4_ROLLOUT.md) Steps 6–7 for the full command sequence. (Update 2026-07-28: apps/site no longer imports the preset — it has its own brand tokens — so this lesson now applies to apps/web builds only.)

## Bare pool reads on RLS'd tables silently return zero rows as trax_app (2026-07-23)
**What happened (C4 Tasks 7/8):** The BFF's subscription write-gate reader (`_sub_status_for` in asgi.py) queried `tenants` on a bare `pool.connection()`. Its bff-level tests passed (in-memory lambda gate) and the pg-level tests used `pg_admin_conn` (superuser, bypasses RLS) — but in production the pool connects as `trax_app` (NOBYPASSRLS) and `tenants` has an RLS policy keyed on the `request.jwt.claims` GUC. A bare connection has no claims set → zero rows → status None → **the gate would have 402'd every write for every tenant**. Caught only because Task 8's implementer empirically tested their own reader with a real trax_app connection before wiring it.
**Rules:**
- **Every asgi-wired pool reader against a tenant-scoped (RLS'd) table MUST use `tenant_conn(pool, tenant_uuid, ...)`** (sets the claims GUC in a transaction) — never a bare `pool.connection()`. The only tables safe to read bare are ones with a policy that doesn't key on claims.
- **`pg_admin_conn` (superuser) tests cannot catch RLS blocks.** Any new reader needs a regression test on the harness's real `trax_app`-role pool: assert the bare-connection pattern returns nothing AND the tenant_conn pattern returns the row. The pair documents WHY tenant_conn is required.
- Smell: "the BFF reads a tenant row at boot fine" is NOT evidence — boot-time reads go through `resolve_tenant_slug`, which is SECURITY DEFINER precisely to bypass this.

## Vercel env vars can exist-but-be-empty — `env ls` proves presence, not validity (2026-07-28)
**What happened (live-demo outage):** aeronta-inventory.vercel.app served the auth-disabled dev-mode build of apps/web — no login page, every `/v1` call 401ing against the auth-required BFF, UI stuck on "Loading dashboard…". `vercel env ls production` showed `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`/`VITE_TENANT_SLUGS` all present (added 7d earlier), but `vercel pull` revealed every value was the EMPTY STRING — `supabase.ts`'s `authEnabled = !!(url && anon)` gate then tree-shakes the whole auth shell out of the bundle, silently producing a "working" build of the wrong app. Diagnosis shortcut that cracked it: the freshly built local bundle hash was IDENTICAL to the live broken one — proving live was faithfully built from current code and the difference had to be build ENV, not code or staleness.
**Rules:**
- After `vercel pull`, verify env value LENGTHS before building: `awk -F= '/^VITE_/ {print $1, length(substr($0,index($0,"=")+1))}' .vercel/.env.production.local` — a `len=2` value is `""`.
- When a deployed site misbehaves but code looks right, build locally and compare bundle hashes with what the alias serves — identical hash = env/config problem, different hash = stale deploy problem.
- An env-gated feature (auth shell) building to the same bundle with unset AND empty-string env means empty-string writes are invisible in every listing tool — only value inspection catches them.
- The Supabase anon key is PUBLIC by design — when it's lost locally, recover it from any deployed client bundle of the same project (aeronta-site's ContactForm chunk carries it) instead of the management API's reveal endpoint.

## The demo tenant's dataset is NOT reproducible from its ingest history (2026-07-28)
`aeronta-demo`'s 57.6k-key dataset came from `trax-io-pg-seed` (snapshot seed), but its only `done` ingest jobs are the tiny C3 smoke sample batches. Anything that "replays the last successful ingest" — the C5 recompute handler, or a re-run of `deploy/aeronta_smoke.py` with `AERONTA_SMOKE_INGEST=1` — REPLACES the seeded dataset with the 3-part sample. Concretely: never redeploy the current-main worker while the nightly `aeronta-nightly-recompute` pg_cron job is scheduled and the tenant's data is seed-sourced; never run the smoke ingest stage against a tenant whose data you want to keep. Re-seed via `trax-io-pg-seed` or a real full-network upload before re-enabling either.

## Root-cause correction + two Vercel env footguns (2026-07-29, demo morning)
**Correction to the 07-28 lesson above:** the VITE_ env vars were not "added empty" — they were created as **SENSITIVE-type** env vars. Sensitive values are write-only outside Vercel: remote builds receive them, but `vercel env pull` returns `""` for them BY DESIGN, so local `vercel build` + `--prebuilt` deploys (mandatory since C4's cross-directory monorepo imports) bake empty strings. That's why C2-era remote-built deploys had working auth and every post-C4 prebuilt deploy shipped the auth-disabled app.
**Rules:**
- **Never store `VITE_*`/`PUBLIC_*` (client-baked) values as sensitive-type env vars** — the browser bundle exposes them anyway, and sensitive-type silently breaks every pull-based/local build. Regular `encrypted` type is correct.
- **Sensitive→encrypted cannot be converted in place** (PATCH with `type` 400s): DELETE the var and POST a new one. The Vercel REST API accepts the CLI's stored bearer (`~/Library/Application Support/com.vercel.cli/auth.json`, CLI 53) — the 2026-07-22 note claiming the CLI session can't be used as a REST bearer is obsolete.
- **`printf value | vercel env add NAME production` on CLI 53 stores an EMPTY string in non-TTY contexts while printing "Added ✓"** — the prompt reads from the TTY, not stdin. Set values via dashboard or REST API from agent sessions; ALWAYS verify afterward with `vercel env pull` + value-length check. A read-back via `GET /v1/projects/{id}/env/{envId}` shows `type` — if it says `sensitive`, pull-verification is impossible and the var must be recreated.
