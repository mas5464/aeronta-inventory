...
## Sonnet subagents nest-delegate and stall; forbid it in the dispatch (2026-07-07)
**What happened:** Across a 25-task subagent-driven build, four sonnet implementers/reviewers spawned their OWN background agents and then stopped to "wait" for them (untracked, sometimes racing the parent on the same files; one left an in-flight mvn process). Each time, a resume message saying "YOU are the implementer — every tool call yours, no background tasks, no monitors, no nested agents, finish synchronously" fixed it immediately.
**Rule:** Put that exact instruction in EVERY implementer/reviewer dispatch prompt from the start (it near-eliminated the pattern once added mid-run). If a subagent's final message says it is "waiting for" another agent, resume it with the do-it-yourself corrective at once; TaskStop stray grandchildren whose work is redundant. Also: an agent killed mid-run usually leaves valid uncommitted work — check `git status` before redoing anything.
