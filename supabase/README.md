...
| Vercel (live) | **https://aeronta-inventory.vercel.app** — apps/web production, `/v1/*` rewrite -> Railway BFF (same-origin); C2 e2e + C3 ingest smoke green through the rewrite 2026-07-21. **Deploy via CLI from `apps/web` — PREBUILT since C4**: `cd apps/web && vercel build --prod && vercel deploy --prebuilt --prod --yes` (plain `vercel deploy` uploads only the CWD, and post-C4 both apps import `../../packages/tailwind-preset` — the remote build fails on the missing module; the `.vercel/` link is gitignored per-checkout — copy `project.json` from another checkout if absent, else the CLI creates a stray project). **The GitHub auto-deploy integration was DISCONNECTED 2026-07-22** (`vercel git disconnect`): it built from the monorepo ROOT (no app there), producing empty deployments that hijacked the prod alias and 404'd the site on every push — see `.claude/memory/lessons.md`. To restore CD, set the project's Root Directory to `apps/web` in the dashboard first, then `vercel git connect`. **2026-07-28 outage + fix (root cause corrected 07-29):** the project's production env vars `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` were stored as **SENSITIVE-type** env vars. Sensitive values never leave Vercel's infrastructure: remote builds see them, but `vercel env pull` (and therefore local `vercel build` + `--prebuilt` deploys, mandatory since C4's monorepo imports) reads them as EMPTY strings — so every prebuilt deploy shipped the auth-disabled dev-mode app (`supabase.ts`'s `authEnabled` gate tree-shakes the auth shell), which the auth-required BFF 401'd — the live site hung on "Loading dashboard…". C2-era plain remote deploys worked, which is why this only bit after the C4 prebuilt switch. **Fixed for good 2026-07-29:** the sensitive vars were deleted and recreated as regular `encrypted` type with real values via the REST API (`POST /v10/projects/prj_WQlrbadCxnWfLQOCteebIIJENzFz/env` with the CLI's `auth.json` bearer — CLI 53's token IS accepted by the REST API); `vercel pull` verified to return the real values (len 42/210). Two footguns to remember: (1) `VITE_*` vars are baked into the public client bundle — NEVER store them as sensitive-type, it breaks prebuilt deploys for zero secrecy gain; (2) `printf value \| vercel env add NAME production` on CLI 53 prints "Added" but stores an EMPTY string in non-TTY contexts — set values via the dashboard or the REST API, and after any env change, `vercel env pull` and check value lengths. |
| Migrations applied (live) | **0001–0015 — ALL repo migrations applied** (verified live 2026-07-28 via `supabase_migrations.schema_migrations`: C1 0001–0005, C2 0006–0007, C3 0008–0009, C4 billing 0010–0012, C5 0013–0014, plus 0015 `resolve_tenant_slug_privileges`). The C4/C5 DB steps happened even though the rest of those rollouts (Stripe objects, Edge Function deploys) did not — the Edge Functions (`create-checkout-session`/`create-portal-link`/`stripe-webhook`) are **NOT deployed** (functions endpoint 404s), i.e. Stripe is intentionally inert; `aeronta-demo` is grandfathered `subscription_status='active'` so the 402 write-gate passes without Stripe. |
| pg_cron | **live and scheduled** (verified 2026-07-28): extension 1.6.4 created, job 1 `aeronta-nightly-recompute` (`0 3 * * *`, `select enqueue_due_recomputes()`) is ACTIVE. ⚠️ The deployed Railway `worker` is still pre-C5 (no `recompute` handler → the nightly job dead-letters harmlessly, but shows as a failed "Scheduled recompute" row in the app's ingest history). **Do NOT redeploy the worker while `aeronta-demo`'s data comes from `trax-io-pg-seed`:** the C5 recompute handler replays the tenant's last `done` *ingest* payload — currently the tiny C3 sample batch — which would replace the 57.6k-key demo dataset. Unschedule the cron (`select cron.unschedule('aeronta-nightly-recompute')`) or re-seed via a real full ingest before updating the worker. |
...
- Demo tenant seeded: `aeronta-demo` ("Aeronta Demo Airline", uuid `753b64bd-9885-4639-b116-8f2c5c497232`) — originally 6 recommendations / 4 part keys from the sample extract. Verified live: RLS deny-without-claims, BFF dashboard/queue/BVR all 200 over `DATABASE_URL`.
- **Re-seeded to the FULL eMRO network 2026-07-22 (current live state):** **41,740 recommendations / 57,605 part keys / 57,605 part contexts** (from the 58,899-key `emro_net_full_snapshot`; 57,605 is the scorable subset — the same "valued 57,605 of 58,899" the BVR discloses). Database is **193 MB of the 500 MB free tier (39%)**. Seeded as `trax_seed` in ~210s. Live perf at this scale is sub-second: `dashboard` 200 in 0.44s (58,899 parts, 38,733 open recs), paged `recommendations` 200 in 0.62s (11,082 pending total).
  - Seeding gotcha: `trax-io-pg-seed` couples the snapshot's `meta.tenant` to the target slug (`from_snapshot_dir` raises on mismatch), and `emro_net_full_snapshot` is labelled `acme`. Do **not** edit that file's label — `docker-compose` mounts the same snapshot with `PLANNER_TENANT=acme`. Instead load and seed as two steps: `PlannerStore.from_snapshot_dir(tenant_id="acme", …)` then `seed_store(pool, store=store, slug="aeronta-demo", …)` — `seed_store` resolves the tenant purely from `slug`.
