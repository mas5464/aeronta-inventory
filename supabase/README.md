...
| Vercel (live) | **https://aeronta-inventory.vercel.app** — apps/web production, `/v1/*` rewrite -> Railway BFF (same-origin); C2 e2e + C3 ingest smoke green through the rewrite 2026-07-21. **Deploy via CLI from `apps/web` — PREBUILT since C4**: `cd apps/web && vercel build --prod && vercel deploy --prebuilt --prod --yes` (plain `vercel deploy` uploads only the CWD, and post-C4 both apps import `../../packages/tailwind-preset` — the remote build fails on the missing module; the `.vercel/` link is gitignored per-checkout — copy `project.json` from another checkout if absent, else the CLI creates a stray project). **The GitHub auto-deploy integration was DISCONNECTED 2026-07-22** (`vercel git disconnect`): it built from the monorepo ROOT (no app there), producing empty deployments that hijacked the prod alias and 404'd the site on every push — see `.claude/memory/lessons.md`. To restore CD, set the project's Root Directory to `apps/web` in the dashboard first, then `vercel git connect`. |
| Migrations applied (live) | **0001–0009** (C1 0001–0005, C2 0006–0007, C3 0008 owner-membership RLS + 0009 `jobs.result`). **0010–0014 exist in this repo (C4 billing 0010–0012, C5 multi-tenant serving + recompute 0013–0014) but have NOT been applied to this live database.** C4's own rollout never happened (`deploy/C4_ROLLOUT.md`); C5's is pending too (`deploy/C5_ROLLOUT.md`), whose own Prerequisite step re-checks this exact live/repo gap before applying anything further — do not trust this table over that live check. |
| pg_cron | verified available on this project: **1.6.4**, `installed_version` is `None` — extension not yet created, no job scheduled. `deploy/C5_ROLLOUT.md` Step 5 creates it and schedules `aeronta-nightly-recompute` (`0 3 * * *`) via plain `cron.schedule(...)` (runs as the caller, `postgres` — **not** `cron.schedule_in_database(..., 'postgres', 'trax_seed')`, which fails on Supabase's non-superuser `postgres` role). |
