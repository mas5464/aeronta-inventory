...
| Vercel (live) | **https://aeronta-inventory.vercel.app** — apps/web production, `/v1/*` rewrite -> Railway BFF (same-origin); C2 e2e + C3 ingest smoke green through the rewrite 2026-07-21. **Deploy via CLI from `apps/web` — PREBUILT since C4**: `cd apps/web && vercel build --prod && vercel deploy --prebuilt --prod --yes` (plain `vercel deploy` uploads only the CWD, and post-C4 both apps import `../../packages/tailwind-preset` — the remote build fails on the missing module; the `.vercel/` link is gitignored per-checkout — copy `project.json` from another checkout if absent, else the CLI creates a stray project). **The GitHub auto-deploy integration was DISCONNECTED 2026-07-22** (`vercel git disconnect`): it built from the monorepo ROOT (no app there), producing empty deployments that hijacked the prod alias and 404'd the site on every push — see `.claude/memory/lessons.md`. To restore CD, set the project's Root Directory to `apps/web` in the dashboard first, then `vercel git connect`. **2026-07-28 outage + fix:** the project's production env vars `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` had been stored as EMPTY strings (present in `vercel env ls`, but `vercel pull` yields `""`), so every build since C2 shipped the auth-disabled dev-mode app, which the auth-required BFF 401'd — the live site hung on "Loading dashboard…". Fixed by writing real values into the local `.vercel/.env.production.local` and deploying prebuilt (bundle `index-DodECuX0.js`, login live). **The STORED project env is still empty — run `vercel env rm`/`vercel env add` for both vars (value source: `PUBLIC_SUPABASE_*` on the aeronta-site project, or the Supabase dashboard API settings) before the next `vercel pull`, or the next build regresses.** `vercel env ls` proves presence, not validity — after any `vercel pull`, sanity-check value *lengths* before building. |
