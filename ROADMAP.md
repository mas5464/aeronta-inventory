...
- [x] **Visual redesign Phase 1 of 4 (dark theme + accent discipline)** — user pointed at an external reference site's UI/UX; decomposed into 4 phases (theme → confidence/rationale treatment → table/badge conventions → nav shell), this slice covers theme only. Dark mode is now a user-toggleable `[data-theme="dark"]` attribute (was OS-only `prefers-color-scheme`) via a new `useTheme` hook (`localStorage`-backed, dark-first default) + a sun/moon toggle in `NavRail`; a new near-black palette (`#0a0a0c`/`#131316`/`#1c1c20`); Tier B moved off blue to teal; a new `action-primary` token pair reserved exclusively for the Approve button (row + drawer), all re-verified against the existing tiered AAA/AA contrast test. Two spec deviations were flagged during planning (kept a separate `action-primary` token instead of remapping ~10 other non-CTA blue usages; left focus rings on the existing accent) — final review endorsed both as sound engineering; deviation #1 was UI-visible and put to the user directly post-ship, who confirmed "keep as shipped." Final review caught + fixed a flash-of-light-theme on every load (dark-first default was applied post-mount by React, not pre-paint — fixed with a tiny inline `<head>` bootstrap script). **Live browser verification (not just tests) then caught a second, more serious bug that 5 prior review passes all missed:** the Approve button's entire bold-fill treatment never actually rendered in either call site — `.approve` (1 class) ties `.actions button` (1 class + 1 type) on specificity and loses, silently falling back to the neutral button style — invisible to all 190 automated tests since they assert click behavior, never computed CSS. Fixed by qualifying `.approve`/`.approve:hover`/`.approve:disabled` with the same ancestor+type prefix so they structurally outrank the generic rule; a repo-wide sweep found no other live instance of the pattern. **190 planner-ui tests (was 184), tsc clean.** — [design](docs/superpowers/specs/2026-07-03-planner-ui-dark-theme-design.md) · [plan](docs/superpowers/plans/2026-07-03-planner-ui-dark-theme.md) — 2026-07-03
- [x] **Visual redesign Phase 2 of 4 (confidence & rationale treatment)** — the QueueTable's confidence column and the Drawer's "why this is queued" text get the reference site's hero treatment. A shared `confidenceTier()` function (high ≥80%, medium 50–79%, low <50%) is the single source of truth for both a new QueueTable percentage badge and a new `ConfidenceHero` component in the Drawer (big colored percentage + reason prose + a "Key findings" evidence list, leading the Drawer body ahead of the part-context strip — the old two-column "current→proposed / why queued" layout is gone, and the header's now-redundant inline confidence text is removed). Confidence-tier colors deliberately avoid the existing Tier A/B/C hues (a collision the plan caught mid-brainstorm): high gets one new violet token (verified 8.78–10.81:1 AAA in both schemes, independently recomputed twice), medium reuses `--text-secondary` (no new color), low reuses the existing danger red. Final review **proactively repeated Phase 1's live-CSSOM verification** (started the dev server, read real computed styles/class hashes in both themes) specifically hunting for the same specificity-bug class that bit Phase 1's Approve button — confirmed it does not recur here. One implementer subagent's connection dropped mid-task (Task 5) after it had already committed correctly; the controller independently verified the resulting code directly against the plan before proceeding, rather than trusting a lost report. **207 planner-ui tests (was 190), tsc clean.** — [design](docs/superpowers/specs/2026-07-03-planner-ui-confidence-rationale-design.md) · [plan](docs/superpowers/plans/2026-07-03-planner-ui-confidence-rationale.md) — 2026-07-03
- [x] **Visual redesign Phase 3 of 4 (table & badge conventions)** — two independent presentation changes, no new backend endpoints. The active queue tab (Pending or Decided) now shows a small pill badge with its row count, reusing `usePlanner`'s existing `total` (the same number `KillSwitchHeader` already displays — zero new data fetching); the inactive tab stays plain text. `QueueTable`'s Part-column criticality indicator changes from a 7px color-only dot (paired with a separate visually-hidden "Criticality N." span) to a 20px numbered circular badge carrying its own `aria-label` — one labeled element instead of two, and a real legibility/accessibility gain since criticality was previously conveyed by color alone. A genuine finding surfaced while grounding the plan: no foreground clears this app's usual 7:1 AAA against the existing 5 criticality colors without retuning them, so a new `--crit-badge-fg` (pure black, declared once — it needs to stay constant across both themes, unlike most tokens) is verified to 4.5:1 AA instead, in a deliberately separate, narrowly-scoped exception array so no other token's threshold is weakened. The user was asked directly and chose to accept AA for this one badge over retuning already-shipped colors; the final review endorsed the call independently and noted an even stronger rationale (the digit likely qualifies as WCAG "large text," whose AA floor is only 3:1, so the exception is conservative relative to spec). **Live-CSSOM specificity verification ran a third consecutive phase** — this time covering `Tabs.module.css`'s new rule for the first time, plus a re-check of `QueueTable.module.css` — no recurrence of the Phase 1 bug class. **219 planner-ui tests (was 207), tsc clean.** — [design](docs/superpowers/specs/2026-07-03-planner-ui-table-badge-conventions-design.md) · [plan](docs/superpowers/plans/2026-07-03-planner-ui-table-badge-conventions.md) — 2026-07-03
- [ ] **Deferred:** weekly Tier-C digest; essentiality mapping + service-level/autonomy-band settings surfaces; SSE real-time; bulk-rollback + confirmation; NL-explanation agent; auth (JWT/Cedar) + DynamoDB persistence; embedded in lighthouse eMRO; **visual redesign Phase 4** (navigation shell); icon-per-evidence-kind in `ConfidenceHero` (pending a closed enum for evidence `kind`); both-tabs-counts + Decided-tab count-accuracy fix (Phase 3 non-goals)
