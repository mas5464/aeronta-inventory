...
- [ ] Strands Supervisor + specialist subagents deployed on AgentCore Runtime (LLM topology, deferred); AgentCore Memory; event lane; CDK
- [ ] Tracked follow-ups: `_PolicyLike` Protocol for the CurrentPolicy seam; `RestWritebackClient` `aclose()` + FAILED-path test; CLI `--apply`-without-URL guard; align `BandAutonomyPolicy` defaults to §6.1; Cedar schema validation; `cedar.py` return-line `# noqa` cleanup
