...
- [x] **Glue loaders for vendor_economics + part_attributes + criticality** — completes materialization for *every required engine input*. `vendor_economics_job` joins `part_master` costs and synthesizes a `DEFAULT` canonical vendor row (preferred-then-cheapest) alongside per-vendor rows; `part_attributes_job`/`criticality_job` mirror the reco bridge derivations (part_class precedence, essentiality→tier map). Spark-verified; CDK packages 6 jobs (42 + 11 tests) — 2026-06-26
- [ ] Glue loaders for the **derived** groups (`lead_time_distribution`, `open_orders_snapshot`, `interchangeable_graph`, `location_graph`); 24-month backfill; DynamoDB online-feature layer; time-travel queries
