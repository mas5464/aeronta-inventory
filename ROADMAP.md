...
- [ ] **C2 pre-flight checklist:** idempotency-key expression index on `writeback_ledger`; `bvr()` single-transaction cache atomicity (stale-serve window under multi-worker); pg/pg-test extra split (testcontainers out of the prod extra); one-migration-runner-per-database rule (Python `apply_migrations` vs. `supabase db push` — never both against the same DB, see `supabase/README.md`); grant `public.custom_access_token_hook(jsonb)` to `supabase_auth_admin` when registering the hook on real Supabase; run the 59K scale gate when a full-network snapshot is regenerated

### C2 — Cloud Deploy
- [ ] BFF + worker on Railway, `apps/web` on Vercel with auth shell + login. Spec: [docs/superpowers/specs/2026-07-20-commercialization-architecture-design.md](docs/superpowers/specs/2026-07-20-commercialization-architecture-design.md)

### C3 — Upload Intake
- [ ] Canonical model v1 column contract, upload UI, ingest job + validation, quota enforcement. Spec: [docs/superpowers/specs/2026-07-20-commercialization-architecture-design.md](docs/superpowers/specs/2026-07-20-commercialization-architecture-design.md)

### C4 — Billing + Marketing Site
- [ ] Stripe tiers/webhooks/portal, Next.js marketing site, self-serve signup funnel. Spec: [docs/superpowers/specs/2026-07-20-commercialization-architecture-design.md](docs/superpowers/specs/2026-07-20-commercialization-architecture-design.md)
